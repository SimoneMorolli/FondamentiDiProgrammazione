...
def count_sub_string_idx(query, corpo):
    ...
    for i in range(len(corpo)):
        if(corpo[i:len(query) + i] == query):
            newList.append((corpo.find(query, i, len(query) + i), corpo.find(query, i, len(query) + i) + len(query) - 1)) 
...
def list_multipli(L):
    ...
    dividedByTwo.extend(dividedByFive)
    return dividedByTwo
...
def cum_sum(L):
    ...
    newElement = 0
    for num in L:
        newElement = num - (-previousNumber)
        previousNumber = newElement
...
def reshape_by_index(L, src, dst):
    ...
    for i in range(max(dst) + 1):
        out.append(None)
    
    for index in range(len(L)):
        indexNumberToTake = src[index]
        newIndex = dst[index]
        print(out)
        out.pop(newIndex)
        out.insert(newIndex, L[indexNumberToTake])
    return out
