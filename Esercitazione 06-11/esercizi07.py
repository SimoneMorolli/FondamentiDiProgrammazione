...
def search_word(filename: str, word: str) -> int:
    contentFile = load_file(filename)
    contentFile = contentFile.lower()
    contentFileFixed = ""
    res = 0
    
    for char in contentFile:
        if char.isalnum():
            contentFileFixed += char        
        elif char == " " or char == "\n":
            contentFileFixed += " "
        
    for element in contentFileFixed.split(" "):
        if element == word:
            res += 1
...
def load_table(filename: str) -> List[Dict[str, str]]:
    newResultList = []
    contentFile = load_file(filename)
    contentFileSplitForRow = contentFile.split("\n")
    listKeyName = contentFileSplitForRow[0].split(" ")
    listKeyName = revomeBlankStringFromList(listKeyName)
    contentFileSplitForRow.pop(0)
    
    for string in contentFileSplitForRow:
        tempDictionary = {}
        valueList = revomeBlankStringFromList(string.split(" "))
        if len(valueList) > 0:
            for i in range(len(listKeyName)):
                tempDictionary[listKeyName[i]] = valueList[i]
            newResultList.append(tempDictionary)
        
    return newResultList
    
    
def revomeBlankStringFromList (a: list) -> list:
    newList = []
    for string in a:
        if string != "":
...
def load_csv(filename: str, empties: bool) -> List[Dict[str, Union[int, str, None]]]:
    ...
    for string in contentFileSplitForRow:
        ...
        for i in range(len(valueList)):
            if valueList[i] == "" and empties == True:
                tempDictionary[listKeyName[i]] = None
            elif valueList[i] == "" and empties == False:
                print("risultato Omesso")
            else:
                tempDictionary[listKeyName[i]] = valueList[i]
            newResultList.append(tempDictionary)
